...
def getPath(node):
    path = ""
    if (
        not node.parentNode().isNull()
        and node.parentNode().nodeType() != QDomNode.DocumentNode
    ):
...
class DomItem:
    def __init__(self, node, row, parent=None):
        ...
        if self.domNode.nodeType() == QDomNode.ElementNode:
            self.editable = (
                self.domNode.childNodes().count() == 1
                and self.domNode.childNodes().at(0).nodeType()
                == QDomNode.TextNode
            ) or (not self.domNode.hasChildNodes())

        self.hasOneGco = False
        if (
            self.domNode.nodeType() == QDomNode.ElementNode
            ...
            self.hasOneGco = (
                ...
                and "gco:"
                in self.domNode.childNodes().at(0).nodeName().lower()
            )
