    ...
    def IsLayerSupport(layer):
        ...
        if layer.type() == QgsMapLayer.VectorLayer:
            if layer.providerType() not in ["ogr", "postgres"]:
                return (
                    False,
                    "Only ogr and postgres vector layers are supported now",
                )
            if layer.providerType() == "ogr":
                if layer.storageType() != "ESRI Shapefile":
                    return (
                        False,
                        "Only 'ESRI Shapefile' ogr provider is supported now!",
                    )
            if layer.providerType() == "postgres":
                if NO_PSYCOPG2:
                    return (False, "psycopg2 libraries are not installed!")
                if not PostgresMetadataProvider.checkExtension(layer.source()):
                    if not PostgresMetadataProvider.installExtension(
                        layer.source()
                    ):
                        return (
                            False,
                            "MetadataPostgis extension are not installed for this DB or connection has failed!",
                        )
...
class PostgresMetadataProvider(RemouteDbMetadataProvider):
    ...
    def getMetadata(self):
        ...
        cur.callproc(
            "GetIsoMetadata",
            [str(self.dsURI.schema()), str(self.dsURI.table())],
        )
        res = cur.fetchone()
    ...
    def setMetadata(self, metadata):
        ...
        cur.callproc(
            "RegisterIsoMetadata",
            [str(self.dsURI.schema()), str(self.dsURI.table()), metadata],
        )
        conn.commit()
        cur.close()
        conn.close()
    ...
    def checkExtension(uri):
        ...
        try:
            ...
            cur = conn.cursor()
            cur.execute(
                "SELECT COUNT(*) FROM pg_class WHERE relname='iso_metadata'"
            )
    ...
    def installExtension(uri):
        ...
        try:
            ...
            print("Exception executing sql: " + str(e))
            return False
