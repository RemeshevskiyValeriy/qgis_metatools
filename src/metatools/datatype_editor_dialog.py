...
class DataTypeEditorDialog(QDialog, FORM_CLASS):
    ...
    def removeDatatype(self):
        if self.datatypeTemplate.name:
            self.datatypeTemplateManager.removeTemplate(
                self.datatypeTemplate.name
            )
            self.reloadTemplatesList()
    ...
    def datatypeChanged(self):
        ...
        self.datatypeTemplate = self.datatypeTemplateManager.loadTemplate(
            templateName
        )
    ...
    def saveTemplate(self):
        ...
        if template.name is None or template.name == "":
            QMessageBox.warning(
                ...
                self.tr(
                    "The name of the data type template must be specified!"
                ),
            )
            return
    ...
    def addKeyword(self):
        keyword, result = QInputDialog.getText(
            self, self.tr("New keyword"), self.tr("Input keyword:")
        )
    ...
    def editKeyword(self):
        if self.lstKeywords.currentRow() < 0:
            QMessageBox.information(
                self, self.tr("Metatools"), self.tr("Select keyword for edit")
            )
            return
        keyword, result = QInputDialog.getText(
            self,
        ...
        if result and keyword:
            self.lstKeywords.item(self.lstKeywords.currentRow()).setText(
                keyword
            )
            self.templateModified()
